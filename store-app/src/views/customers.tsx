...
function KhataLedger({
  ...
  // Keep form type in sync with mode
  ...
  const onSubmit = (values: TxFormValues) => {
    addTx.mutate(
      { id: customerId, data: values },
      {
        ...
        onError: () =>
          toast({ title: "Transaction nahi hua", variant: "destructive" }),
      }
    );
  };
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html><head><title>Khata Statement - ${detail.name}</title>
      <style>body{font-family:system-ui,sans-serif;margin:0;padding:24px;color:#111}
      h2{margin:0 0 4px}p{margin:0 0 2px;color:#555;font-size:13px}
      table{width:100%;border-collapse:collapse;margin-top:20px}
      th{background:#f1f5f9;padding:10px 12px;text-align:left;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;color:#64748b}
      .due{font-size:20px;font-weight:700;color:#b45309}
      .footer{margin-top:24px;font-size:12px;color:#94a3b8;border-top:1px solid #eee;padding-top:12px}
      </style></head><body>
      <h2>${shopName}</h2>
      <p>Khata Statement</p>
    ...
    <div className="flex flex-col gap-0" style={{ maxHeight: "80vh" }}>
      ...
      <div className="rounded-xl border bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200 p-4 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="text-right">
          ...
          {detail.totalDue > 0 && (
            <Badge className="bg-amber-100 text-amber-700 border-amber-200 text-[10px] mt-1">Pending</Badge>
          )}
          {detail.totalDue === 0 && (
            <Badge className="bg-green-100 text-green-700 border-green-200 text-[10px] mt-1">Clear</Badge>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <Button
          size="sm"
          className="gap-1.5 bg-positive hover:bg-green-700 text-white"
        ...
        <Button
          size="sm"
          variant="outline"
          className="gap-1.5 border-warning text-warning hover:bg-amber-50"
      ...
      {mode && (
        <div
          className={cn(
            ...
            mode === "payment" ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"
          )}
        >
          <div className="flex items-center justify-between">
            <p className="font-semibold text-sm">
              {mode === "payment" ? "💰 Payment Entry" : "📦 Credit / Udhaar Entry"}
            </p>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-wrap gap-3 items-end">
                ...
                render={({ field }) => (
                  <FormItem className="flex-[2] min-w-[160px]">
                    <FormLabel className="text-xs">
                      {mode === "payment" ? "Note (optional)" : "Saman / Item name"}
                    </FormLabel>
                    <FormControl>
                      <Input
                        placeholder={mode === "payment" ? "e.g. Cash payment" : "e.g. Rice, Toor Dal"}
                        {...field}
                        className="h-9"
              ...
              <Button
                ...
                className={cn(
                  "h-9",
                  mode === "payment" ? "bg-positive hover:bg-green-700" : "bg-warning hover:bg-amber-600"
                )}
      ...
      <div className="flex-1 overflow-auto rounded-xl border">
        ...
        {ledgerRows.length === 0 ? (
          ...
          <div className="divide-y">
            {ledgerRows.map((tx, i) => (
              <div
                key={tx.id}
                className="grid grid-cols-[1fr_2fr_auto_auto] gap-0 hover:bg-muted/20 transition-colors"
              ...
              >
                ...
                <div className="px-4 py-3 flex items-start gap-2 min-w-0">
                  <div
                    className={cn(
                      ...
                      tx.type === "credit" ? "bg-amber-100 text-warning" : "bg-green-100 text-positive"
                    )}
                  >
                    {tx.type === "credit" ? (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {tx.type === "credit" ? "Udhaar / Credit" : "Payment Received"}
                    </p>
                  </div>
                </div>
                ...
                <div className="px-4 py-3 text-right whitespace-nowrap">
                  <p
                    className={cn(
                      ...
                      tx.type === "credit" ? "text-warning" : "text-positive"
                    )}
                  >
                ...
                <div className="px-4 py-3 text-right whitespace-nowrap">
                  <p
                    className={cn(
                      "font-bold text-sm",
                      tx.balance > 0 ? "text-warning" : "text-positive"
                    )}
                  >
        ...
        {ledgerRows.length > 0 && (
          <div className="grid grid-cols-[1fr_2fr_auto_auto] border-t bg-muted/40">
            <div className="px-4 py-3 col-span-3 text-sm font-bold text-foreground">
              Total Balance Due
            </div>
            <div className="px-4 py-3 text-right">
              <span
                className={cn(
                  ...
                  detail.totalDue > 0 ? "text-warning" : "text-positive"
                )}
              >
                ₹{detail.totalDue.toFixed(2)}
              </span>
  ...
  const onAdd = (values: CustomerFormValues) => {
    createCustomer.mutate(
      { data: values },
      {
        ...
        onError: () =>
          toast({ title: "Customer add nahi hua", variant: "destructive" }),
      }
    );
  };
    ...
    <div className="space-y-5" data-testid="page-customers">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {customersWithDue > 0 ? (
              <span className="text-warning font-semibold">
                {customersWithDue} ka ₹{totalDue.toFixed(0)} pending
              </span>
            ) : (
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onAdd)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. Ravi Kumar" data-testid="input-customer-name" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. 9876543210" data-testid="input-customer-phone" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Pata (Optional)</FormLabel>
                    <FormControl>
                      <Input {...field} placeholder="e.g. Gandhi Nagar" data-testid="input-customer-address" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              >
                {createCustomer.isPending ? "Saving..." : "Customer Save Karein"}
              </Button>
            </form>
          </Form>
        </DialogContent>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground rounded-xl border border-dashed">
          ...
          <p className="font-medium">Koi customer nahi</p>
          <p className="text-sm mt-1">Upar "Add Customer" se customer add karein</p>
        </div>
      ) : (
        <div className="space-y-2">
          {customers.map((customer) => (
            <div
              ...
              className={cn(
                "group flex items-center gap-4 rounded-xl border bg-card px-4 py-3.5 transition-all hover:shadow-sm",
                customer.totalDue > 0 ? "border-amber-200 hover:border-amber-300" : "hover:border-border/80"
              )}
            >
              {/* Avatar */}
              <div
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {customer.totalDue > 0 && (
                    <Badge className="bg-amber-100 text-amber-700 border-amber-200 text-[10px] font-medium px-1.5">
                      Due
                    </Badge>
                  )}
                </div>
              ...
              <div className="text-right shrink-0">
                <p
                  className={cn(
                    ...
                    customer.totalDue > 0 ? "text-warning" : "text-positive"
                  )}
                >
                  ₹{customer.totalDue.toFixed(0)}
                </p>
                <p className="text-[10px] text-muted-foreground">
                  {customer.totalDue > 0 ? "Pending" : "Clear"}
                </p>
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 gap-1.5 text-xs border-primary/30 text-primary hover:bg-teal-50"
                  onClick={() => setLedgerId(customer.id)}
                  data-testid={`button-ledger-${customer.id}`}
                >
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-opacity"
