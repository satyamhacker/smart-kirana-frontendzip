  ...
  return useQuery({
    ...
    queryFn: () => {
      const url = params?.search
        ? `/api/products?search=${encodeURIComponent(params.search)}`
        : "/api/products";
...
// ─── Purchase Hooks (kept for compatibility) ──────────────────────────────────

export function useListPurchases() {
  return useQuery({
    queryKey: getListPurchasesQueryKey(),
    queryFn: () => Promise.resolve([] as Purchase[]),
  });
}
...
export function useCreatePurchase() {
  return useMutation({
    mutationFn: async ({ data }: { data: PurchaseInput }) =>
      Promise.resolve({ ...data, id: 0, createdAt: new Date().toISOString() } as Purchase),
  });
}
...
export function useGetLowStockReport() {
  return useQuery<Product[]>({
    ...
    queryFn: () => apiFetch<Product[]>("/api/reports/lowstock"),
  });
}
