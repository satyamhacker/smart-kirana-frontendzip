...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-72 flex flex-col border-r border-sidebar-border bg-sidebar shadow-xl transition-transform duration-300 md:hidden",
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-5">
          ...
          <button className="rounded-lg p-1.5 hover:bg-muted" onClick={() => setMobileOpen(false)}>
            <X className="h-5 w-5" />
          </button>
        </div>
